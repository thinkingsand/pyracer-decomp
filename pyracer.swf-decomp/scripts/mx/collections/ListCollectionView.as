package mx.collections
{
   ...
   public class ListCollectionView extends Proxy implements ICollectionView, mx.collections.IList, IMXMLObject
   {
      ...
      public function getItemIndex(param1:Object) : int
      {
         ...
         if(Boolean(this.localIndex) && Boolean(this.sort))
         {
            _loc3_ = this.sort.findItem(this.localIndex,param1,mx.collections.Sort.FIRST_INDEX_MODE);
            if(_loc3_ == -1)
            {
               return -1;
            }
            _loc4_ = this.sort.findItem(this.localIndex,param1,mx.collections.Sort.LAST_INDEX_MODE);
            _loc2_ = _loc3_;
            while(_loc2_ <= _loc4_)
            {
         ...
         if(this.localIndex)
         {
            ...
            while(_loc8_ < param1.length)
            {
               ...
               if(this.filterFunction == null || this.filterFunction(_loc9_))
               {
                  if(this.sort)
                  {
                     _loc7_ = this.mx_internal::findItem(_loc9_,mx.collections.Sort.ANY_INDEX_MODE,true);
                     if(_loc6_)
                     {
                        _loc5_ = _loc7_;
                        _loc6_ = false;
